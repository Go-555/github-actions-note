...
class MemoResearcher:
    ...
    def _generate_memos(self, count: int) -> List[dict]:
        ...
        prompt = (
            "あなたは生成AIビジネスを中心に幅広いビジネス活用や事業、副業、テクノロジー事情に詳しいリサーチャーです。"
            f"キーワード '{keyword}' に関連する最新ニュース、ビジネス活用事例、個人向けの生成AI活用、スモールビジネスへの活用などを"
        ...
        return [
            {
                ...
                "bullets": [
                    "最新のリリース情報をウォッチ",
                    "導入企業の成功事例を調査",
                    "個人の体験談や生成AIに関するノウハウ、稼ぎ方や最新プロダクト事情調査",
                ],
            }
            for i in range(count)
        ]
