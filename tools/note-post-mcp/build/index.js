...
async function postToNote(params) {
    const { markdownPath, thumbnailPath, statePath = DEFAULT_STATE_PATH, isPublic, screenshotDir = path.join(os.tmpdir(), 'note-screenshots'), timeout = DEFAULT_TIMEOUT, } = params;
    // Markdownファイルを読み込み
    ...
    try {
        ...
        if (thumbnailPath && fs.existsSync(thumbnailPath)) {
            ...
            if (cropperEl && saveEl) {
                await Promise.race([
                    page.waitForFunction((el) => getComputedStyle(el).pointerEvents === 'none', cropperEl, { timeout }),
                    page.waitForFunction((el) => !el.disabled, saveEl, { timeout }),
                ]);
            }
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.trim().startsWith('```')) {
                // コードブロック全体（```から```まで）を収集
                const codeBlockLines = [line]; // 開始行を含める
                let j = i + 1;
                // 終了行まで収集
